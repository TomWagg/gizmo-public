...
When using specific modules, the relevant code papers must also be cited: for example, papers using the MHD version of the code should cite the MHD methods paper: Hopkins & Raives 2015 ("Accurate, Meshless Methods for Magneto-Hydrodynamics"). Anisotropic diffusion operators (conduction, viscosity, radiation, cosmic rays, non-ideal MHD, passive scalar diffusion) should site Hopkins 2016 ("Anisotropic Diffusion in Mesh-Free Numerical Magnetohydrodynamics"). The citations required for the various code modules are all given in the "Config.sh (Setting Compile-Time Options)" section of this Guide, where the modules are described. Tom Wagg and Michael Ryan have written a short python script to read in configuration files and generate a citation/acknowledgements section. See `get_citation.py` in the scripts folder.
...
First, get a copy of the source code from the [Public Code Site](https://github.com/pfhopkins/gizmo-public) or the [development (private) Repository](https://bitbucket.org/phopkins/gizmo).
...
**GALSF\_FB\_FIRE\_AGE\_TRACERS**: Add explicit tracking of tracer species for arbitrary stellar yields as a function of age from different stellar populations, developed by Andrew Emerick (methods paper Wetzel et al. 2023, in prep, in the meantime cite arXiv:2203.00040). enabled by default with the FIRE defaults for newer FIRE versions.
...
**SINGLE\_STAR\_FB\_JETS**: Adds feedback from proto-stellar jets. This acts as as kinematic outflows with a specified outflow rate and velocity set in the same manner as black hole-driven accretion-disk outflows. This leverages the `BH_WIND_SPAWN` module, see the description in the black hole notes for the details of the numerical methods. The run-time parameters `BAL_f_accretion` or `Sink_f_accretion` (both set the same variable, your choice of the convention to use), `BAL_v_outflow` or `Sink_v_outflow`, `BAL_internal_temperature` or `Cell_Spawn_Temperature`, and `Cell_Spawn_Mass_ratio`, are used, as is standard with this module (see the parameterfile notes below for the blackhole/sink-particle modules) to determine the mechanical and thermal energy-loadings (as well as e.g. optional magnetic or cosmic-ray loading) for the jets. The jets, by default, are collimated with a very small opening angle at launch (they almost immediately will expand as they propagate), along the direction of the sink accreted specific angular momentum. Cite Su et al, arXiv:2102.02206, and Grudic et al., arXiv:2010.11254, for the methods here.

**SINGLE\_STAR\_FB\_WINDS**: Adds continuous main-sequence mechanical feedback from single stellar sources accounting for OB/AGB/WR winds. This adds the parameter `Cell_Spawn_Mass_ratio_MS` which can be set for the cell-spawning for this mode of feedback analogous to `Cell_Spawn_Mass_ratio`, but allowing you to specify these separately. This has two possible algorithmic channels it can adopt, depending on other flags that are set. With STARFORGE parent flag[s] enabled, this will following STARFORGE methods (Grudic+ arXiv:2010.11254). Otherwise, this will follow a simpler Castor, Abbot, & Klein scaling, for type=4 particles representing single stars, using the standard `GALSF_FB_MECHNICAL` algorithms in code, for which you should cite Hopkins et al. 2018MNRAS.477.1578H. Users interested in modifying the relevant scalings or active particle types should modify the code appropriately in the `stellar_evolution.c` file.
...
Within the default `COOLING` module there are many sub-modules. Some of these depend on enabling specific radiation-hydrodynamics modules, discussed below. For example, if photo-electric or Lyman-Werner or Compton (X-ray) or dust infrared or ionizing wavelengths are evolved explicitly in the radiation-hydrodynamics, they all automatically are incorporated into this `COOLING` routine. However there are some additional switches here that are important for many simulations. All of the options below are part of the public code, but please follow the relevant citation guidelines.

**COOL\_METAL\_LINES\_BY\_SPECIES**: Uses full multi-species-dependent metal-line cooling tables. The tables correspond to CLOUDY runs following Wiersma et al. 2009 (MNRAS, 393, 99). The relevant pre-computed tables can be downloaded [here](http://www.tapir.caltech.edu/~phopkins/public/spcool_tables.tgz) or on the BitBucket download page. This requires that METALS and COOLING are on. When active, the code will follow 11 species independently (H, He, C, N, O, Ne, Mg, Si, S, Ca, Fe, in addition to the "total" metallicity), with heating/cooling computed for each assuming its optically thin (and self-consistently computing the ionization states). Users should cite Wiersma et al. 2009 (MNRAS, 393, 99) and Hopkins et al. 2017 (arXiv:1702.06148). To use this module, one must download and unpack the actual cooling tables and have them (un-zipped and un-tarred) in a folder named `spcool_tables` placed within the run-time directory of GIZMO. Again, tables can be download either [here](http://www.tapir.caltech.edu/~phopkins/public/spcool_tables.tgz) or on the GIZMO public or private (either one) bitbucket code repository website. 

**COOL\_LOW\_TEMPERATURES**: Adds fine-structure and molecular cooling, to extend the cooling curves down to ~10 K. The cooling rates at low-temperatures are explicitly metal-species-dependent if `COOL_METAL_LINES_BY_SPECIES` is also enabled (any version of this requires `METALS` enabled). These follow a detailed ensemble of processes including molecular and fine structure cooling, dust-gas collisional cooling, cosmic ray heating, photo-electric and other terms. Some terms are taken from fits to CLOUDY models, but most are separated because each can depend on the local abundances and radiation fields in different ways. The list of terms, and the sources for each expression used, are given in arXiv:1702.06148 and updated in arXiv:2203.00040. This requires COOLING is enabled; any cooling associated with metal species requires METALS be enabled and cells have a defined metallicity field. Note that while this includes an approximate treatment of optically-thick cooling, it is designed for simulations of star and galaxy formation and should be extrapolated to dense systems (e.g. stars, planetary atmospheres, etc) with caution and does not even attempt to include physics such as neutrino cooling. Anyone using this should cite Hopkins et al. 2017 (arXiv:1702.06148) and 2023 (arXiv:2203.00040); a complete list (and description of the physics and fitting functions) of all cooling physics included is given in the Appendix B of the former paper [the parts enabled by this piece of the code include all components/terms below $10^{4}$ K] and since 2020 use the updates given in section 3 of the latter paper.

**COOL\_MOLECFRAC**: If `COOL_LOW_TEMPERATURES` is enabled, the code will extend the ionized+atomic molecular network to include Hydrogen (H2) molecular chemistry. This switch (set to a given value) determines how the H2 fractions and cooling rates are estimated. Generally speaking, higher values involve more complicated models for H2, which may require more additional fields are set as they also depend on quantities like the local abundances, dust-to-gas ratio, FUV radiation field, etc. The values here correspond to: (1) Default to an extremely-oversimplified scaling for H2 fraction as a function of density and temperature, set by fits to the temperature below which gas at a given density becomes molecular from cloud simulations presented in Glover+Clark 2012. (2) Estimate H2 fraction with Krumholz & Gnedin 2010 fitting function, assuming simple scalings of radiation field, clumping, and other factors with basic gas properties so this is a function only of surface density (estimated with a Sobolev-type approximation) and metallicity, truncated at low values (or else it gives non-sensical answers). (3) Estimate H2 fractions from the approximate large-scale expressions from Gnedin & Draine (2014ApJ...795...37G). use the sub-grid final expression calibrated to ~60pc resolution simulations with equilibrium molecular chemistry and post-processing radiative transfer from Gnedin & Draine 2014 (See their Eqs. 5-7). This depends on density, metallicity, column density, and the local FUV field. (4) Estimate H2 fractions from the approximate large-scale expressions from Krumholz, McKee, & Tumlinson (2009ApJ...693..216K). This is an equilibrium model of spherical clouds of internally uniform properties exposed to incident radiation. Depends on density, column density, metallicity, and incident FUV field. (5) Estimate molecular fractions for thermochemistry+cooling from local equilibrium H2 formation+destruction with clumping and self-shielding (Hopkins et al. arXiv:2203.00040). This solves the full set of H2 formation and destruction equations from Nickerson, Teyssier, & Rosdahl et al. 2018 (including dust and gas-phase formation, 3-body formation and collisional H-H2 and H2-H2 dissociation, Lyman-Werner and ionizing and cosmic ray dissociation). For the radiation terms this accounts for line self-shielding with turbulent and cell line-blanketing terms from Gnedin & Draine 2014, and dust self-shielding in the radiation hydro module, with clumping factors and the turbulence model using a combination of the code local turbulent (eddy) estimator plus a standard lognormal distribution at the cell scale. The network is solved assuming local equilibrium fully-implicitly. (6) This network is identical to (5), the only difference is that the H2 fraction is explicitly evolved, allowing it to have non-equilibrium values. Citations for these stem from Hopkins et al. 2023, 2023MNRAS.519.3154H or arXiv:2203.00040 -- cite that paper if you use any of these.

**COOL\_UVB\_SELFSHIELD\_RAHMATI**: This replaces the self-shielding approximation used for the UV background with a slightly different but more accurate approximation (Hopkins et al. arXiv:2203.00040). This follows Rahmati et al. 2013MNRAS.431.2261R, who calibrate to variation radiation-transport calculations, but with an additional simple correction to account for the fact that the fitting functions from that paper decline (unphysically) too slowly at high gas densities, which leads to artificial suppression of molecular gas even at arbitrarily high densities. 


Note that there has unfortunately been some confusion about the physics and applications of the GIZMO low-temperature cooling modules. Specifically, Wibking and Krumholz 2023 (WK23) stated (incorrectly) that there was an 'error' in the implementation of cooling in GIZMO, and showed some cooling curves in their Appendix A which were reproduced directly in e.g. Kim et al. 2023 (arXiv:2210.08024). But the issue noted in WK23 was not in fact, as the authors acknowledge in the text, an error or bug in the code -- indeed, the authors validated directly that GIZMO was, in their test problem, solving the thermochemistry correctly given the input physics/assumptions for the flags turned on. Specifically, WK23 considered a test problem where they simulated a homogeneous box illuminated by a constant background field (no local sources, no explicit or implicit radiation transport), turning on the pre-2020 version of the `COOL_LOW_TEMPERATURES` flag with no other physics (not including any soft X-ray, cosmic ray, or shock/hydrodynamic sources of ionization, nor ionization of heavier species beyond H and He), using the popular self-shielding prescription generally used for cosmological simulations (across many different codes) at the time derived from e.g. Schaye et al. 2001 (2001ApJ...559..507S) to shield that uniform background. WK23 (Appendix A) showed that this leads -- as it should by construction -- to exponential suppression of the free electron fraction at high densities in primarily neutral gas. They then showed that this (via its indirect effects on e.g. photo-electric heating) has relatively modest effects on the equilibrium temperature and pressure of the neutral, dense ISM at densities $n_{\rm H} >1\,{\rm cm}^{-3}$ and temperatures $T<10^{4}\,{\rm K}$, equivalent to shifting $\langle T(n_{H})\rangle$ by a factor of a few in $n_{H}$ or $T$, compared to a model which included no self-shielding whatsoever (only optically-thin treatment of the meta-galactic and galactic background in the cold, dense gas). This is again expected, and users should note this is generally smaller than other physical effects which GIZMO includes options for, such as different choices of how to approximate the molecular gas formation/destruction chemistry, treatment of the radiation fields and their thermo-chemical coupling, explicit cosmic ray physics, etc. Unfortunately this cooling curve was labeled 'FIRE-2 Cooling' in WK23, despite the fact that the physics and numerical module choices used in their setup are quite different from what is actually enabled in the published FIRE-2 papers and, as noted in e.g. arXiv:1702.06148, 1811.12462, 2203.00040 and other papers, the thermal pressure/temperatures in the WNM and CNM are actually much closer in actual FIRE-2 simulations to what WK23 argued was their best 'full physics' reference model in their paper (interested users can obtain the public FIRE-2 snapshots available through [`http://fire.northwestern.edu`](http://fire.northwestern.edu)). Moreover, post-2020, the default behavior of the `COOL_LOW_TEMPERATURES` flag in GIZMO has been extended with a variety of physics detailed and extensively validated against state-of-the-art chemical networks including those in Glover and Clark, Wise and Abel, GRACKLE, CHIMES, KROME, and many more, explicitly for neutral gas at densities as high as $10^{18}\,{\rm cm}^{-3}$ in 2021MNRAS.506.2199G, 2023MNRAS.519.3154H. This includes explicit treatment of sources of ionization like cosmic rays, extending the radiation networks enabled by any of the user-desired ionization-related modules (and of the relevant RHD modules including those used in FIRE/STARFORGE and many other projects) to include ionizations from the entire frequency range, expanding the treatment of hydrodynamic/shock ionization (which was included pre-2020 but not in the specific test problem in WK23) to non-equilibrium shock ionization, self-consistent ionization of heavier elements, and inclusion many other relevant physics in these regimes such as electron depletion onto dust grains, detailed cross-coupling between molecular and dust chemistry, H- anion chemistry, and more. Of course, users also have the option to turn off or modify the self-shielding treatment entirely, or to use the more modern self-shielding calibration from `COOL_UVB_SELFSHIELD_RAHMATI`, or (ideally if important to the problem) to simply use self-consistent radiation transport with local sources, since the fundamental argument of WK23 was not specific to GIZMO (nor FIRE) but rather to the popular Schaye et al. 2001 self-shielding prescription. 

However, we generally note that many dozens of papers (including, but certainly not limited to, GIZMO papers such as 2012MNRAS.421.3488H, 2014MNRAS.445..581H, 2018MNRAS.478.3653O, 2018MNRAS.480..800H, 2020MNRAS.491.3702H, 2023MNRAS.519.3154H) have shown that in most cosmological and/or galaxy formation simulations, orders-of-magnitude larger changes than those noted in WK23 (including simply turning off cooling entirely below $10^4$ K) have extremely minor effects on almost all galaxy dynamical and star formation properties (and 2023MNRAS.519.3154H specifically compared results including/excluding the terms discussed in WK23). For *individual* star formation simulations using e.g. the `SINGLE_STAR_DYNAMICS` modules, or if the primary focus of your study is predicting these atomic/molecular properties in detail, then of course more detailed/accurate treatments are desired and recommended. For these, we recommend using the modern 'full-physics' version of GIZMO, but (crucially) avoiding the ambiguity of simple self-shielding prescriptions entirely by using one of the many available explicit radiation-hydrodynamics treatments in the code (which are coupled directly to these thermochemistry modules).
...
**IO\_SINKS\_ONLY\_SNAPSHOT\_FREQUENCY**: Outputs some snapshots with a reduced data set including only sink particles. Determines the number of snapshots with reduced data (sinks only) per full snapshots (gas+sinks+other), e.g., setting this to 2 means 2/3 of the snapshots will be reduced, 1/3 will have full data. Setting this to 0 disables it. Developed by David Guszejnov.
...
**CRFLUID\_BINCENTERED\_TRANSPORT**: Turn this on to use the older bin-centered cosmic ray transport scheme for spectrally-resolved cosmic rays (where properties like fluxes and diffusion coefficients are evaluated at the geometric mean or center of each spectral momentum bin), instead of the more accurate scheme which prevents bin-to-bin artifacts as developed in Hopkins 2022 arXiv:2202.05283. If not enabled, all spectral cosmic ray simulations will use the Hopkins 2022 scheme by default.
...
For convenience and quality-of-life reasons, as well as historical compatibility, some of these parameters will accept any of multiple names in the parameterfile (e.g. there might be a name which is more descriptive and useful based on **GIZMO** conventions and this user guide, but you can also often use the old GADGET-2 name for a parameter if you want, for parameters that were defined in those older codes). You just choose one to use based on your preference.
...
As a result of this, a 'minimal' parameterfile can be as short as the following:
    ...
    TreeRebuild_ActiveFraction  0.01        % fraction active for build 
...
**ICFormat** (or `Initial_Conditions_Format`): This flag selects the file format of the initial conditions read in by the code upon start-up. A value of 1 selects the standard unformatted binary file-format of GADGET-2 (which is identical to the unformatted binary format used for GIZMO), while a value of 2 or 4 selects a more uncommonly-used variant of this simple binary format (also identical between GADGET/GIZMO). A value of 3 selects the use of HDF5 binary instead (again, same for GADGET/GIZMO). The structure of these files will be discussed in a separate section below. This should always match the format of the file named in "InitCondFile". If not specified, this will default to type 3 (hdf5), but many codes widely-used to generate ICs will generate format = 1 ICs, so you may need to set this in many of your applications.

**SnapFormat** (or `Snapshot_Format`): A flag that specifies the file-format to be used for writing snapshot files. The possible format choices are the same as for ICFormat. It is therefore possible to use different formats for the initial conditions and the produced snapshot files. If not specified, this will default to type 3 (hdf5).
...
**TimeOfFirstSnapshot** (or `Simulation_Time_of_First_Snapshot`): This variable selects the time for the first snapshot (if OutputListOn=0). Note that the time units are a=1/(1+z) for comoving integrations, while they are physical time for non-comoving integrations. (Ignored if `OutputListOn` is set to `1`, otherwise defaults to the first time in the simulation set by `TimeBegin`).

**TimeBetSnapshot** (or `ScaleFac_Between_Snapshots`): If the user chooses not to specify a pre-set list of snapshot times, then after a snapshot has been written, the time for the next snapshot is determined in one of two ways. For non-cosmological simulations, recall that the time coordinate is just time, in physical units, and so the next snapshot time is determined by adding `TimeBetSnapshot` to the last snapshot time -- i.e. the snapshots will be uniformly spaced in time with spacing equal to `TimeBetSnapshot`. For cosmological simulations (comoving integration), on the other hand, the time coordinate is the scale factor (a), and this represents the **scale factor between snapshots**, i.e. the next snapshot time is determined by multiplying the current snapshot time by this variable (hence the alternate name `ScaleFac_Between_Snapshots`). This will lead to a series of snapshots which are uniformly space in log(a). This variable is ignored if `OutputListOn` is set to `1`.
...
**UnitLength\_in\_cm** (or `UnitLength_in_centimeters`): This sets the internal length unit in cm/h, where $H_{0} = 100\,h$ km/s/Mpc. The above choice is convenient for galaxy-scale simulations – it sets the length unit to 1.0 kpc/h. (Default: 1, only for simulations which do not need any dimensions).

**UnitMass\_in\_g** (or `UnitMass_in_grams`): This sets the internal mass unit in grams/h. The above choice is convenient, it sets the mass unit to $10^{10}\,M_{\odot}\,h^{-1}$. (Default: 1, only for simulations which do not need any dimensions).

**UnitVelocity\_in\_cm\_per\_s** (or `UnitVelocity_in_centimeterspersecond`): This sets the internal velocity unit in cm/sec. The above choice is convenient – it sets the velocity unit to km/sec. Note that the specification of `UnitLength_in_cm`, `UnitMass_in_g`, and `UnitVelocity_in_cm_per_s` also determines the internal unit of time. The definitions made above imply that in internal units the Hubble constant has a numerical value independent of h and hence HubbleParam. For the numerical examples above, the Hubble constant has always the value 0.1 in internal units, independent of h, and the Hubble time is always 10 in internal units, with one internal time unit corresponding to 9.8e8 yr/h. However, you are free to choose a different system of units if you like. Note also that this implies that for purely gravitational dynamics, the code will not need to know the value of "h" at all. HubbleParam is kept in the parameterfile only because additional physics in the hydrodynamical sector may require it. (Default: 1, only for simulations which do not need any dimensions).

**UnitMagneticField\_in\_gauss** (or `UnitMagneticField_in_Gauss_for_ICSnapshotIO`): This sets the internal magnetic field unit in Gauss. Needed to be able to read/write input and output files. While actually working, the code will use the appropriate matching units for code pressure, etc. (Default = $\sqrt{4\pi}$, only for simulations which do not need any dimensions).

**GravityConstantInternal** (or `GravityConstant_inCodeUnits`): The numerical value of the gravitational constant G in internal units depends on the system of units you choose. For example, for the numerical choices made above, G=43007.1 in internal units. For `GravityConstantInternal=0` the code calculates the value corresponding to the physical value of G for you. Sometimes, you might want to set G yourself. For example, by specifying `GravityConstantInternal=1`, `UnitLength_in_cm=1`, `UnitMass_in_g=1`, and `UnitVelocity_in_cm_per_s=1`, one obtains a ‘natural’ system of units. Note that the code will nevertheless try to use the ‘correct’ value of the Hubble constant even in this case, so you should always set GravityConstantInternal=0 in cosmological integrations or any problem with gravity and real units, as opposed to some special numerical/code internal units (those are the kinds of problems this flag is designed for). (Default: `0`)


    %---- Cosmological parameters 
    ComovingIntegrationOn 1 % is it cosmological? (yes=1, no=0)
    BoxSize               10000.0 % in code units
    Omega_Matter          0.27 % =0 for non-cosmological
    Omega_Lambda          0.73 % =0 for non-cosmological
    Omega_Baryon          0.044 % =0 for non-cosmological
    OmegaRadiation        0 % =0 for non-cosmological
    HubbleParam           0.7  % little 'h'; =1 for non-cosmological runs

**ComovingIntegrationOn** (or `Cosmological_Simulation_On`): This flag enables or disables comoving integration in an expanding universe. For ComovingIntegrationOn=0, the code uses plain Newtonian physics with vacuum or periodic boundary conditions. Time, positions, velocities, and masses are measured in the internal system of units, as specified by the selected system of units. For ComovingIntegrationOn=1, the integration is carried out in an expanding universe, using a cosmological model as specified by Omega0, OmegaLambda, etc. In this cosmological mode, coordinates are comoving, and the time variable is the expansion factor itself. If the code has not been compiled with the `BOX_PERIODIC` makefile option, the underlying model makes use of vacuum boundary conditions, i.e. density fluctuations outside the particle distribution are assumed to be zero. This requires that your particle distribution represents a spherical region of space around the origin. If `BOX_PERIODIC` is enabled, the code expects the particle coordinates to lie in the interval [0, BoxSize]. (Default: 0, i.e. non-cosmological).

**BoxSize** (or `Box_Size_In_Code_Units`): The size of the periodic box (in code units) encompassing the simulation volume. This parameter is only relevant if the `BOX_PERIODIC` makefile option is used. Particle coordinates need to lie in the interval [0, BoxSize] in the initial conditions file. Ignored if no code settings require a defined box size
     
**Omega\_Matter** (or `Omega0`): Cosmological matter density parameter in units of the critical density at z=0. Generally relevant only for comoving integration. (Default in non-cosmological simulations: `0`, will not influence results in this case).

**OmegaLambda** (or `Omega_Lambda`): Cosmological vacuum energy density (cosmological constant) in units of the critical density at z=0. Relevant for comoving integration, but can in principle be set for a non-periodic box to reflect uniform expansion. For a geometrically flat universe, one has Omega0 + OmegaLambda = 1. For simulations in Newtonian space (non-cosmological, non-expanding), this parameter has to be set to zero. (Default in non-cosmological simulations: `0`, will not influence results in this case).

**OmegaBaryon** (or `Omega_Baryon`): Baryon density in units of the critical density at z=0. Relevant only for comoving integration. (Default in non-cosmological simulations: `0`, will not influence results in this case).

**OmegaRadiation** (or `Omega_Radiation`): Radiation energy density in units of the critical density at z=0. Relevant only for comoving integration. (Default: `0`, will not influence results in this case).
     
**HubbleParam** (or `Hubble_Param_Little_h`): Value of the Hubble constant in units of 100 km/s/Mpc. This is only used in cosmological integrations when conversions to cgs units are required (e.g. for radiative cooling physics). Note however that the value of HubbleParam is not needed (in fact, it does not enter the computation at all) in purely collisionless simulations. This is because of the definition of GIZMO’s system of units, which eliminates an explicit appearance of the value of the Hubble constant in the dynamical equations. (Default in non-cosmological simulations: `1`, will not influence results in this case).
...
Gravity is softened with a spline, as outlined in the code paper. The softenings quoted here all refer to the roughly equivalent Plummer softening length. Note that for the spline that is actually used, the force will be exactly Newtonian beyond `1/KERNEL_FAC_FROM_FORCESOFT_TO_PLUMMER` (value defined in the code in kernel.h, =2.8 for example for a cubic spline kernel) times this value. The code will tell you exactly what this factor is on startup in the stdout file. The softening lengths are in internal (code) length units. For cosmological simulations, that means these refer to co-moving units.
...
**ShearViscosityCoeff/BulkViscosityCoeff**: These define the shear ($\eta$) and bulk viscosity ($\zeta$) coefficients, for physical Navier-Stokes viscosity (VISCOSITY on). If `VISCOSITY_BRAGINSKII` is on, the relevant coefficients are calculated physically; these are simply dimensionless multipliers. Again when this is set, the default value the code will adopt (if this is not set) is =1. If, for example, you run a non-MHD version of the calculation, people often set a multiplier $<1$ to represent suppression by tangled magnetic fields. Of course, since the code correctly handles anisotropic viscosity with MHD, you should really use that!. If `VISCOSITY_BRAGINSKII` is off, these are used directly (they should be given in code units: we use *dynamic viscosities* here, so the units are mass / (time $\times$ length). 
...
**CosmicRayDiffusionCoeff**: Set the diffusion coefficients for cosmic ray diffusion/streaming (`COSMIC_RAY_FLUID` on), assuming you are using the fixed-scattering-rate models `CRFLUID_DIFFUSION_MODEL=0`, as opposed ot the models where the CR scattering rates are set dynamically according to different physical models. For the single-bin CR models, this is just the diffusion (note that the CRs can also stream at the Alfven speed, as described in Chan et al. and Hopkins et al. methods papers on this as noted in the Config file descriptions for these flags). For the multi-bin (resolved) models, this normalizes the diffusivity and scattering rate at a rigidity of 1 GV. The value should be in code units. Recall that the observationally-constrained effective anisotropic (parallel) diffusion coefficient measured at the Solar circle from a wide variety of data (detailed spectra, spallation products, radioactive products, gamma-ray emission, ionization, synchrotron) is 3e29 cm2/s, in cgs. 

**CosmicRay\_SNeFraction** (or `CosmicRay_SNeEnergyFraction`): If `COSMIC_RAY_FLUID` and `GALSF_FB_MECHANICAL` are both on with the default FIRE physics implementation active as well, then a fraction (typically calculated and observationally inferred to be $\sim 0.1$) of the initial supernova kinetic energy of each explosion is deposited into cosmic rays.
...
Note that there are a variety of other grain parameters that can be enabled if the user specifies certain config flags. As described in the `GRAIN_RDI_TESTPROBLEM` description, this enables 

    %-------------- parameters for additional grain simulations (GRAIN_RDI_TESTPROBLEM on)
    Grain_Charge_Parameter             1  % dimensionless charge (3*q_grain/[4pi*c*R_grain^2*sqrt(rho_box)])
    Dust_to_Gas_Mass_Ratio          0.01  % dust/gas by mass: ~0.01 for all grains, for solar abundance
    Vertical_Grain_Accel             5e7  % vertical radiative accel in code units (cs^2/Lbox)
    Vertical_Grain_Accel_Angle         0  % angle (in deg) between grain accel and grav accel
    Vertical_Gravity_Strength        1e5  % vertical gravity in code units: = L_box/H_gravity_scale_length
    Grain_Q_at_MaxGrainSize          0.2  % Q normalization at maximum grain size
    Grain_Absorbed_vs_Total_Extinction 0  % absorbed (vs scattered) fraction

**Grain\_Charge\_Parameter** is the dimensionless grain charge parameter, $\tilde{\phi}_{\rm max}$, from Lee et al. 2017MNRAS.469.3532L and Seligman et al 2019MNRAS.485.3991S, defined for the largest grains. 

**Dust\_to\_Gas\_Mass\_Ratio** the mean dust to gas mass ratio of the box (grain super-particle masses will be renormalized assuming their initial total mass in the box is unity, to give this value - don't use this unless your ICs are actually set up appropriately)

**Vertical\_Grain\_Accel**: the value of the acceleration from external forces (e.g. radiation) on dust grains (in code units), normalized to the largest grains (for simulations with live radiation-dust-magnetohydrodynamics, this normalizes the incident radiation flux to the parameter $a_{0}$ as described in Hopkins et al. arXiv:2107.04608)

**Vertical\_Grain\_Accel\_Angle**: an arbitrary angle in degrees between the direction of the grain acceleration and gravity (defining e.g. anti-parallel for this equal to zero)

**Vertical\_Gravity\_Strength**: the value of the externally imposed, constant analytic gravitational force, in code units

**Grain\_Q\_at\_MaxGrainSize**: value of the dimensionless grain extinction efficiency $Q$ (cross section relative to geometric, at the wavelength of interest), for the largest grains, if `RT_OPACITY_FROM_EXPLICIT_GRAINS` is set 

**Grain\_Absorbed\_vs\_Total\_Extinction**: fraction of incident radiation absorbed (1-albedo), assumed for grains, if `RT_OPACITY_FROM_EXPLICIT_GRAINS` and `RT_GENERIC_USER_FREQ` are set. 
...
Note that *all* of these FIRE parameters have sensible default values (the values in the parameter block above), which will be automatically used if the relevant code modules are turned on but the parameters are not specifically set by the user.
    ...
    BAL_f_accretion              0.5        % fraction of gas swallowed by sink (BH_WIND options; alt name Sink_f_accretion)
    BAL_v_outflow                30000	    % velocity (code units) of outflow from sink (BH_WIND options; alt name Sink_v_outflow)
    BAL_internal_temperature     1.0e4      % internal temperature (K) of BAL outflow (BH_WIND_SPAWN; alt name Cell_Spawn_Temperature)
    BAL_wind_particle_mass       1.e-10     % mass of 'virtual wind particles' in code units (BH_WIND_SPAWN; use Cell_Spawn_Mass_ratio if SINGLE_STAR or BH_SCALE_SPAWNINGMASS_WITH_INITIALMASS is active)
    BH_CosmicRay_Injection_Efficiency 0.1   % fraction of accretion energy into cosmic rays (BH_COSMIC_RAYS; alt name Sink_CosmicRay_Injection_Efficiency)
...
**BAL\_f\_accretion**/**BAL\_v\_outflow** (or **Sink\_f\_accretion**/**Sink\_v\_outflow**): These control the model for accretion disk or broad-absorption line winds from the BH (compiler flags beginning with `BH_WINDS`). `BAL_f_accretion` (or `Sink_f_accretion`) controls the mass-loading of the winds: a fraction `BAL_f_accretion` of the mass in the inner accretion disk is assumed to be accreted, while (`1-BAL_f_accretion`) is blown out in the wind, with launch velocity `BAL_v_outflow` (or `Sink_v_outflow`) in km/s. See the Config.sh discussion, or the `notes_blackholes` file, for more details. 

**BAL\_internal\_temperature**/**BAL\_wind\_particle\_mass** or (**Cell\_Spawn_\Temperature**/**Cell\_Spawn\_Mass\_ratio**): These options are specific to the module `BH_WIND_SPAWN`, and set the internal temperature (in K) and virtual particle mass (in code units) of the virtual "wind particles" spawned by the BH wind. Note that as described above, if the `SINGLE_STAR` modules are active or one sets the flag `BH_SCALE_SPAWNINGMASS_WITH_INITIALMASS`, then the spawn mass is not fixed to a single absolute value but a value equal to `Cell_Spawn_Mass_ratio` times the initial formation mass of the sink particle doing the spawning.
...
If that really doesn't answer your question, please check our Issues and/or Discussions fora at the public GIZMO GitHub site [here](https://github.com/pfhopkins/gizmo-public).  There are various posts archived there, with code issues, bugs and their fixes, feature requests and their modules that were developed, all maintained. First check if someone else has already asked the same question, then (if not), please post and join the discussion -- that's what its for, and other users most likely have answers for you. Note that unfortunately, the previous Google Groups system we had used had to be closed permanently owing to security and spam issues (and we were not able to import the older posts).

If you still cannot get an answer, the best thing to do is to find another experienced user, and ask them directly. Many such users are active in the development community, and of course you can reach out to colleagues, collaborators, and members of the community who have published many papers using GIZMO. And of course if you can't find anyone there, you can contact the code developers (Phil or the salient developers of the modules in question, which are listed in detail in the `Template_Config.sh` file, and User Guide descriptions).
...
**Parameterfile Settings:** The params file also has a couple settings that can have a big impact. (1) `TreeRebuild_ActiveFraction` or `TreeDomainUpdateFrequency` (just alternate names for the same parameter): this requires more hand-tuning from experience than almost any other parameter. See its description: decreasing it will force the code to rebuild the domain decomposition more frequently, and can reduce load imbalances. But making it too frequent will cause it to bottleneck the code. A good 'rule of thumb' is that if the code is spending comparable time on imbalances (`treeimbal`+`densimbal`+`hydimbal` in `cpu.txt`) and domain decomposition (`domain` in `cpu.txt`), then this is set about right (but if both of these are very large, this is obviously an issue). (2) `PartAllocFactor`: this sets how flexible the code can allocate particles to allow larger memory imbalances in order to reduce load imbalances. You generally want it as large as memory allows. (3) Other parameters like force softening, neighbor number, time between snapshots or statistics, etc, are generally small corrections, unless they are set to inappropriate values and cause problems. For example, setting a force softening much too-small means the timesteps are artificially small; setting it too large will introduce large imbalances as the particles need to perform direct-$N^{2}$ gravity operations between every neighbor within their force-softening kernels.
